...
class MY_Upload extends CI_Upload
{
    ...
    public function upload_to_s3()
    {
		...
		$CI->load->library('cisssintegration_lib');//Load the aws integration library if it is not loaded
        $s3     = new AmazonS3();
		
		//If buckets have more than one period in the name (e.g. myapp.mydomain.com). The documentation in the AWSDK states that this can result in an SSL verification error on some servers. To fix it we are writing below code.
		//if(substr_count($this->bucket_name, '.') > 1)
			//$s3->enable_path_style(TRUE);
			
        $opt = array(
            'fileUpload' => $this->file_temp,
        ...
        do {
			$r = $s3->create_object($this->bucket_name, $this->upload_path . $this->file_name, $opt); 
            if ($r->isOK()) {
                return true;
            }
            sleep($sleep);
    ...
    {
        $CI =& get_instance();
        $CI->load->library('cisssintegration_lib');
        $s3     = new AmazonS3();
		...
		$opt = array(
            'fileUpload' => $this->file_resource,
			'contentType' => $this->file_type
        ); // use only standard storage as data may lose
		//echo $this->s3_path . $this->file_name;
        //echo "uplaoding";exit;
		//echo $this->s3_path . $this->file_name;exit;
        $try   = 1;
        $sleep = 1;
        do {
            $r = $s3->create_object($this->bucket_name, $this->s3_path . $this->file_name, $opt);
