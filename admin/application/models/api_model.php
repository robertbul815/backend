...
class Api_model extends CI_Model {
	...
	public function remove_user($remove_id) {
		...
		$query_user = $this->db->get_where('tr_users', array('user_id' => $remove_id));
		$current_user = $query_user->row_array();

		$query_posts = $this->db->get_where('tr_posts', array('post_user_id' => $current_user['user_id']));
		$posts = $query_posts->result_array();
		foreach($posts as $post) {
			...
			$this->db->delete('tr_comments', array('comment_post_id' => $post['post_id']));
		}
		$this->db->delete('tr_posts', array('post_user_id' => $current_user['user_id']));
		$this->db->delete('tr_comments', array('comment_user_id' => $current_user['user_id']));
		$this->db->delete('tr_follows', array('follow_user_id' => $current_user['user_id']));
		$this->db->delete('tr_likes', array('like_user_id' => $current_user['user_id']));
		...
		$this->db->delete('tr_users', array('user_id' => $remove_id));
	...
	public function remove_post($remove_id) {
		...
		$query_post = $this->db->get_where('tr_posts', array('post_id' => $remove_id));
		...
		$this->db->delete('tr_posts', array('post_id' => $remove_id));
		$this->db->delete('tr_comments', array('comment_post_id' => $post['post_id']));
